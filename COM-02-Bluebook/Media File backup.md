1. You have two servers Source and Backup server.
2. Make insure that the source server can access the remote server with `ssh-key`. 
3. Create the following backup script. 
	```bash 
	#!/bin/bash
	# Variables
	...
	SSH_KEY="/root/.ssh/id_ed25519"  # Make sure to set this to the correct path
	ARCHIVE_DIR="/tmp/backup_archives"  # Temporary directory for storing archives
	...
	# Function to delete backups older than 2 days
	cleanup_old_backups() {
	    echo "[Cleanup] Removing backups older than yesterday (keeping only today and yesterday)" >> "$LOGFILE"
	
	    # Get today's and yesterday's folder names (YYYY-MM-DD format)
	    TODAY=$(date +%Y-%m-%d)
	    YESTERDAY=$(date -d "yesterday" +%Y-%m-%d)
	
	    # Dry-run: List folders to be deleted (exclude today/yesterday)
	    echo "[Dry Run] Folders to be deleted:" >> "$LOGFILE"
	    ssh -i "$SSH_KEY" "haitham@$REMOTE_IP" \
	        "find '$REMOTE_BASE' -mindepth 1 -maxdepth 1 -type d -name '20[0-9][0-9]-[0-1][0-9]-[0-3][0-9]' \
	        | grep -v -E '$TODAY|$YESTERDAY'" \
        >> 	"$LOGFILE" 2>&1 || { echo "ERROR: SSH/find failed during dry-run." >> "$LOGFILE"; exit 1; }
	
	    # Actual deletion (remove folders NOT matching today/yesterday)
	    if ssh -i "$SSH_KEY" "haitham@$REMOTE_IP" \
	        "find '$REMOTE_BASE' -mindepth 1 -maxdepth 1 -type d -name '20[0-9][0-9]-[0-1][0-9]-[0-3][0-9]' \
	        | grep -v -E '$TODAY|$YESTERDAY' \
	        | xargs rm -rf" \
        >> 	"$LOGFILE" 2>&1; then
	        echo "[Cleanup] Success: Kept '$TODAY' and '$YESTERDAY' backups." >> "$LOGFILE"
	    else
	        echo "[Cleanup] FAILED to delete old backups!" >> "$LOGFILE"
	        exit 1
	    fi
	}
	...
	rm -rf "$ARCHIVE_DIR"
	```
4. Create cronjob which run every 5 minutes. 
	```bash 
	crontab -e 
	```
	add the following 
